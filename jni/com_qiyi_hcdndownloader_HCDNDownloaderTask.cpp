#include "com_qiyi_hcdndownloader_HCDNDownloaderTask.h"#include "com_qiyi_hcdndownloader_HCDNDownloaderCreator.h"#include "CubeTaskCallBackImpl.h"#include <stdlib.h>#include "PublicMethod.h"#include "stdafx.h"#ifdef __cplusplusextern "C" {  #endif    JNIEXPORT jboolean JNICALL Java_com_qiyi_hcdndownloader_HCDNDownloaderTask_StartNative(JNIEnv *env, jobject jtask)    {        if(NULL == env)            return JNI_FALSE;        IHCDNDownloaderTask* task = gettaskptrbyjava(env,jtask);
        ...
        return brt?JNI_TRUE:JNI_FALSE;    }    JNIEXPORT jboolean JNICALL Java_com_qiyi_hcdndownloader_HCDNDownloaderTask_StopNative(JNIEnv *env, jobject jtask,jint stop_type)    {        if(NULL == env)            return JNI_FALSE;			        IHCDNDownloaderTask* task = gettaskptrbyjava(env,jtask);
...
        return brt?JNI_TRUE:JNI_FALSE;    }    JNIEXPORT jlong JNICALL Java_com_qiyi_hcdndownloader_HCDNDownloaderTask_GetSpeedNative(JNIEnv *env, jobject jtask,jint speed_type)    {        if(NULL == env)            return 0;        unsigned int realtime_speed = 0;        unsigned int mean_speed = 0;        IHCDNDownloaderTask* task = gettaskptrbyjava(env,jtask);
        ...
        long long filssize = static_cast<long long>(task->GetFileSize());        return filssize;    }    JNIEXPORT jlong JNICALL Java_com_qiyi_hcdndownloader_HCDNDownloaderTask_GetDownloadSizeNative(JNIEnv *env, jobject jtask)    {        if(NULL == env)            return 0;        IHCDNDownloaderTask* task = gettaskptrbyjava(env,jtask);
        if(NULL == task)
        {
            psl::logger::CLogger::PrintA(LOG_HCDNDOWNLOADER_TASKEX,psl::logger::_Fatal, "%s %d: NULL == taskptr\r\n",__FUNCTION__,__LINE__);
            return 0;
        ...
        long long filssize = static_cast<long long>(task->GetDownloadSize());        return filssize;    }    JNIEXPORT void JNICALL Java_com_qiyi_hcdndownloader_HCDNDownloaderTask_SetSpeedLimitNative(JNIEnv *env, jobject jtask ,jint jspeed)    {        if(NULL == env)            return;        IHCDNDownloaderTask* task = gettaskptrbyjava(env,jtask);
        if(NULL == task)
        {
            psl::logger::CLogger::PrintA(LOG_HCDNDOWNLOADER_TASKEX,psl::logger::_Fatal, "%s %d: NULL == taskptr\r\n",__FUNCTION__,__LINE__);
            return;
